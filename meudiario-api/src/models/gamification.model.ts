...
export const CHALLENGES = [
  { id: 1, description: 'Escreva 5 anotações esta semana', rewardPoints: 50, kind: 'notes', target: 5 },
  { id: 2, description: 'Registre seu humor todos os dias desta semana', rewardPoints: 40, kind: 'moods', target: 7 },
  { id: 3, description: 'Escreva uma anotação com pelo menos 3 tags', rewardPoints: 30, kind: 'note-tags', target: 3 },
  { id: 4, description: 'Escreva uma anotação com mais de 200 caracteres', rewardPoints: 25, kind: 'note-length', target: 200 },
  { id: 5, description: 'Mantenha um streak de pelo menos 3 dias esta semana', rewardPoints: 35, kind: 'streak', target: 3 },
] as const
...
export const getMilestoneSteps = (streak: number): { next7: number; next14: number; next30: number; next60: number; next100: number } => ({
...
export const toBadgeResponse = (badge: { id: string; name: string; description: string }, unlockedAt: Date | null): BadgeResponse => ({
  id: badge.id,
  name: badge.name,
