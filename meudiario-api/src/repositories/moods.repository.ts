...
export class MoodsRepository {
  ...
  upsertMoodForDate(userId: string, date: Date, value: number, noteId?: string | null): Promise<Mood> {
  ...
  async listHistory(userId: string, filters: MoodHistoryFilters): Promise<{ moods: Array<Pick<Mood, 'id' | 'value' | 'date' | 'noteId'>>; total: number }> {
    const where = {
      userId,
  ...
  async listByDateRange(userId: string, start: Date, end: Date): Promise<Array<Pick<Mood, 'id' | 'value' | 'date' | 'noteId'>>> {
