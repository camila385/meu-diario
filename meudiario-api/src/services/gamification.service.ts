...
import { getChallengeForCurrentWeek, getChallengeProgressPercent, getCurrentIsoWeekRange, getProgressSummary } from '@/utils/gamification'
...
export class GamificationService {
	...
	async getBadges(userId: string): Promise<BadgeResponse[]> {
		...
		const unlockedById = new Map(userBadges.map((userBadge) => [userBadge.badgeId, userBadge.unlockedAt]))
	...
	async getWeeklyChallenge(userId: string): Promise<WeeklyChallengeResponse> {
		...
		if (challenge.kind === 'moods') {
			const moods = await this.moodsRepository.listByDateRange(userId, weekRange.start, weekRange.end)
	...
	async awardForNote(userId: string, input: CreateNoteRequest): Promise<void> {
		...
		const lastActivity = gamification.lastActivity ? toUtcDayStart(gamification.lastActivity) : null
		const diffDays = lastActivity ? Math.floor((today.getTime() - lastActivity.getTime()) / 86400000) : null
	...
	private async enqueueBadgeChecks(userId: string): Promise<void> {
		...
		const notesCount = await this.notesRepository.listNotes(userId, { page: 1, limit: 1 }).then((result) => result.total)
		const moodHistory = await this.moodsRepository.listHistory(userId, { page: 1, limit: 1 }).then((result) => result.total)
