...
import type { CreateMoodRequest, MoodHistoryQuery, MonthlyMoodSummaryQuery } from '@/validators/moods.validator'
import { toMoodHistoryItem, toMoodResponse, type MonthlyMoodSummary, type MoodResponse, type WeeklyMoodSummary } from '@/models/mood.model'
...
export class MoodsService {
  ...
  async createMood(userId: string, input: CreateMoodRequest): Promise<MoodResponse> {
    ...
    if (input.noteId) {
      ...
      if (noteOwner.userId !== userId) {
        throw new ForbiddenError('Acesso negado. Apenas o proprietário pode vincular esta anotação.')
      }
  ...
  async listHistory(userId: string, query: MoodHistoryQuery): Promise<{ data: ReturnType<typeof toMoodHistoryItem>[]; meta: ReturnType<typeof buildMeta> }> {
  ...
  async monthlySummary(userId: string, query: MonthlyMoodSummaryQuery): Promise<MonthlyMoodSummary> {
